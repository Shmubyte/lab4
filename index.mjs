...
app.get('/nasapod', async (req, res) => {
   try {
      const response = await fetch(`https://api.nasa.gov/planetary/apod?api_key=9mUzIkhlZCZaOoMfspg7jMmwZCZ4LiRHtkgkambD&date=2025-11-28`);
      const data = await response.json();
      
      console.log(data); // See the data in your terminal
      res.render('nasapod', { nasapod: data });
   } catch (error) {
      console.error('Error fetching nasapod:', error);
